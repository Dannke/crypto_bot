...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Add debug to see what regime is detected and what signals are generated
        import crypto_bot.pipeline.portfolio_fusion as fusion_mod
        import crypto_bot.strategy.portfolio_strategies as strategies_mod
        ...
        def debug_fuse(self, intents, regime):
            ...
            for intent in intents:
                for pi in intent.intents:
                    print(f"  Intent: {pi.symbol} {pi.side} weight={pi.target_weight}")
            return original_fuse(self, intents, regime)

        fusion_mod.RegimeGatedFusion.fuse = debug_fuse

        # Patch CSM evaluate_market to add debug
        original_evaluate = strategies_mod.CrossSectionalMomentumStrategy.evaluate_market
        
        def debug_evaluate(self, snapshot, state):
            print(f"DEBUG CSM: evaluate_market called, as_of_ms={snapshot.as_of_ms}")
            print(f"  Symbols: {list(snapshot.candles_by_symbol.keys())}")
            for sym, bars in snapshot.candles_by_symbol.items():
                print(f"  {sym}: {len(bars)} bars, last_close={bars[-1].close}")
            result = original_evaluate(self, snapshot, state)
            print(f"  CSM result: {len(result.intents)} intents")
            for pi in result.intents:
                print(f"  Intent: {pi.symbol} {pi.side} weight={pi.target_weight}")
            return original_evaluate(self, snapshot, state)

        strategies_mod.CrossSectionalMomentumStrategy.evaluate_market = debug_evaluate
        ...
        def debug_fuse(self, intents, regime):
            print(f"DEBUG FUSE: Regime={regime.regime}, strategy={intents[0].strategy_name if intents else None}")
            for intent in intents:
                for pi in intent.intents:
                    print(f"  Intent: {pi.symbol} {pi.side} weight={pi.target_weight}")
            return original_fuse(self, intents, regime)
