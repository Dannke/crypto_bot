...
def candles_uptrend(sym: str, tf: str, n: int, sp: float) -> list[Candle]:
    ...
    for i in range(n):
        ...
        result.append(Candle(timestamp=ts, open=o, high=h, low=l, close=c, volume=1000.0))
        price = c
    return result
...
def make_config(max_open: int) -> Config:
...
def max_concurrent(db_path: str) -> int:
            ...
            SELECT opened_at_ms,
                   (SELECT COUNT(*) FROM positions p2
                    WHERE p2.opened_at_ms <= p1.opened_at_ms
                      AND (p2.closed_at_ms IS NULL OR p2.closed_at_ms > p1.opened_at_ms)) AS open_count
            FROM positions p1
        )
    """).fetchone()
    con.close()
...
def total_positions(db_path: str) -> int:
    con = sqlite3.connect(db_path)
    cnt = con.execute("SELECT COUNT(*) FROM positions").fetchone()[0]
    con.close()
    return cnt
...
for limit, label in [(3, "WITH fix (max=3)"), (999, "WITHOUT fix (max=999 → no limit)")]:
    db = run_bt(limit)
    mc = max_concurrent(db)
    tp = total_positions(db)
    print(f"{label}: total_positions={tp}, MAX(concurrent)={mc}")
