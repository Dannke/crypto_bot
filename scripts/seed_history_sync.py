"""Sync seed historical OHLCV from exchange into local SQLite DB (uses ccxt sync, bypasses aiodns issue)."""
...
from crypto_bot.data.exchange_sync import MarketDataClientSync
from crypto_bot.storage.db import Database, Repositories


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Seed historical OHLCV data from exchange (sync)")
    parser.add_argument("symbol", help="Trading pair (e.g. BTC/USDT)")
    parser.add_argument("timeframe", help="Timeframe (e.g. 1h, 4h, 1d)")
    parser.add_argument("--start", default=None, help="Start datetime (ISO format, e.g. 2024-01-01)")
...
def _run(
    ...
    with MarketDataClientSync(config) as client:
        while since < end_ms:
            raw = client.fetch_ohlcv(symbol, timeframe, limit=limit, since=since)
            if not raw:
                break
