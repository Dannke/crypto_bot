...
### ⚠ Расхождение: реализация выходит через механизм, которого нет в модели

**Обнаружено 2026-09-20** прогоном на реальных данных (train-сегмент
2025-01-01..2025-07-02, 9 символов, 23783 бара каждый, 70 сделок). Walk-forward
не запускался; результатов на test-сегменте не существует.

Зарегистрированная выше модель исполнения знает три механизма: post-only вход,
post-only выход по цели реверсии, рыночный fallback по таймауту. **Stop-loss и
take-profit в этом документе не упоминаются ни разу.** В коде же
`PortfolioExecutor.open_position` безусловно вызывает `SLTPCalculator.calculate`
для каждой позиции, и `check_positions_range` проверяет эти уровни на каждом баре.

Фактическое распределение выходов (идентично в обоих режимах исполнения):

| closed_by | сделок | доля |
|---|---:|---:|
| `stop_loss` | 47 | 67% |
| `take_profit` | 21 | 30% |
| `rebalance` | 2 | 3% |
...
Собственные выходы стратегии не сработали ни разу. Причина структурная:
`backtester.py:542` — портфельный тик выходит рано, если не наступил ребаланс,
поэтому условия `reversion` / `time_stop` проверяются **раз в 12 часов**, тогда
как `check_positions_range` с SL/TP отрабатывает **на каждом часовом баре**.
Двенадцать попыток против одной при горизонте удержания 48 часов.

**Экономическое следствие.** `check_positions_range` закрывает позицию с
`is_maker=False`, то есть taker. Вход остаётся maker (подтверждено: все 70
сделок записаны как `order_type='limit'`). Round-trip перестаёт быть
симметричным:

| модель | round-trip | daily | annual | вердикт |
|---|---:|---:|---:|---|
| как зарегистрировано (maker/maker) | 6.0 bps | 12.0 bps | 43.80% | WARN |
| фактическая механика (maker/taker) | 18.0 bps | 36.0 bps | **131.40%** | **FAIL** |
| по наблюдённой смеси (68/70 taker) | 17.7 bps | 35.3 bps | 128.90% | **FAIL** |

Команда для средней строки (скрипт принимает симметричные ставки, поэтому
18 bps round-trip задаются как 9 bps на сторону):

```bash
python scripts/estimate_mr_turnover.py --max-positions 4 --holding-hours 48   --rebalance-hours 12 --fee-bps 6 --slippage-bps 3
```
```
    Round-trips/day: 2.00
    Daily: 36.0 bps
    Annual: 131.40%
  FAIL: Annual cost drag > 100% (131.4%)
```

То есть при выходах, смоделированных так, как они реально исполняются, v4 не
проходит собственный cost-gate — не WARN на 43.8%, а FAIL на 131.4%.

**Решение НЕ принимается этим документом, и вот почему.** Есть два взаимно
исключающих способа устранить расхождение:

- **A.** Зарегистрированная модель — это и есть замысел. Тогда SL/TP не должны
  навешиваться на MR-позиции (либо должны стоять заведомо шире цели реверсии), а
  условия выхода стратегии должны проверяться достаточно часто, чтобы успевать
  срабатывать. Maker-экономика сохраняется, cost-gate остаётся 43.80% WARN.
- **B.** Реализация — это и есть замысел. Тогда SL/TP должны быть
  зарегистрированы как часть стратегии, а cost-gate обязан считать taker на ~97%
  выходов. Результат — 131.40%/год, FAIL, и v4 отклоняется собственным
  cost-gate ещё до walk-forward.

Выбор варианта A на том основании, что он сохраняет проходящий гейт, был бы
подгонкой — тем же нарушением, что и снятие инварианта валидации, только на
уровне механики исполнения. Выбор должен опираться на то, чем стратегия должна
быть по замыслу, и принятый результат гейта принимается любым.

Отдельно: расхождение существует не с v4, а как минимум с v3 — тот же
`SLTPCalculator` вызывался и тогда. Цифра 43.8%/год не описывала реализованную
систему ни в одной редакции.

### Решение по расхождению (принято 2026-09-20, до измерения результата)

**Выбран вариант A.** ATR-стопы на MR-позициях признаны наследством общего
портфельного слоя, а не замыслом стратегии. Зарегистрированная модель исполнения
остаётся в силе как описание замысла: выход по реверсии, по time-stop или по
рыночному fallback при таймауте. SL/TP как способ выхода из mean_reversion_v0
исключаются.

**Основание — принадлежность механизма, а не его стоимость.** Стопы никогда не
регистрировались ни в одной редакции документа (v1–v4), их параметры
(`take_profit_risk_multiple`, ATR-множители) живут в общем `risk`-блоке и
применяются ко всем портфельным стратегиям одинаково. Ни одна редакция
pre-registration не обсуждала их как часть гипотезы о возврате к среднему.

**Что зафиксировано до прогона.** На момент принятия решения известно, что
вариант A сохраняет cost-gate на 43.80%/год (WARN), а вариант B уводит его в
131.40%/год (FAIL). Решение принято по принадлежности механизма; результат
гейта принимается любым. Каким окажется распределение выходов и n_trades после
снятия стопов — на момент записи этого раздела **неизвестно**, измерение идёт
после. Порядок проверяем по истории git: коммит с решением предшествует коммиту
с измерением.

**Реализация.** `PortfolioExecutor._sltp_exits_enabled` = False, когда активна
`mean_reversion_v0`; `check_positions_range` для неё не выполняется. Уровни
`stop`/`take` продолжают вычисляться и храниться, поскольку колонки
`positions.stop` и `positions.take` объявлены `NOT NULL` — сделать их
необязательными требует отдельной последовательной миграции, вынесено в
открытые пункты.

**Что этим решением НЕ меняется.** Частота проверки условий выхода остаётся
прежней — раз в тик ребаланса, то есть раз в 12 часов, тогда как документ о
частоте проверки выходов молчит. Менять её сверх принятого решения было бы
изменением поведения стратегии без зарегистрированного основания. Если после
снятия стопов окажется, что 12-часовая сетка сдвигает срабатывание time-stop
(48 баров = 48 ч) на величину до 12 часов — это фиксируется отдельным
измерением и решается отдельно.

### Решение по коллизии сторон (принято 2026-09-21, до измерения частоты)

**Выбран вариант A: инкумбент побеждает.** Символ, уже удерживаемый на одной
стороне, исключается из кандидатов противоположной.

**Суть проблемы уже, чем «кто побеждает».** Коллизия возникает только тогда,
когда движение внутри одного интервала ребаланса перепрыгивает зону выхода
`[-0.5, +0.5]` целиком, не будучи зафиксированным ни на одном тике: вошли при
z=−3.2, следующая проверка застаёт z=+3.5, реверсия не сработала. Для крипты
это не теоретическая аномалия — экономический механизм самой гипотезы v4
(ликвидационные каскады, давление фандинга) порождает ровно такие гэпы.

**Основание выбора.** A продолжает существующее поведение стратегии: символы,
вышедшие по `exited_symbols`, уже исключаются из повторного входа тем же
способом. B (переворот позиции) вводит новый механизм оборота, не
зарегистрированный ни в одной редакции, и платит полный round-trip на каждом
перевороте.

**Что зафиксировано до измерения.** Частота коллизий на исторических данных на
момент принятия решения **неизвестна**. Если она окажется высокой, выбор
материально влияет на оборот и, следовательно, на cost-gate; если низкой —
выбор непринципиален. Измерение выполняется после этого коммита и
докладывается отдельно как sanity-check, а не как основание выбора.

**Реализация.** `MeanReversionStrategy.evaluate_market`: после фильтра
`exited_symbols` добавлен фильтр по `held = set(keep_long) | set(keep_short)`.
