...
**Дата:** 2026-09-26
**Статус:** кандидат на заморозку — ожидает вердиктов `preregistration-guardian` и
`cost-gate-auditor` (CLAUDE.md, правило 6); walk-forward **не запускался**. Заморозка — коммит,
который после вердиктов переводит этот статус в «заморожен»; `config/settings.yaml`
коммитится вместе с документом.
...
в момент входа — средний `|r_4h|` в началах `k = 3.0` на train, 558.2 bps (документ измерения,
раздел 4).
